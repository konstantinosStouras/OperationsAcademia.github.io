...
(function () {
  ...
  /** The ID token the rules read, re-minted before this form writes
      ...
      oa-accounts.js has no such export, and a profile must not fail for
  ...
  /** A REFUSED PROFILE NAMES NO CAUSE THIS PAGE CANNOT KNOW.
      ...
      accepting profiles yet — its database rules have not been published",
    ...
    form.addEventListener('submit', function (e) {
      ...
      OAAccounts.whenSignedIn(function (user) {
        /* THE TOKEN THE RULES READ, FIRST OF ALL. Both the Storage rule on the
           CV and the Firestore rule on the profile are gated on `verified()`,
           which reads `email_verified` off the ID TOKEN — cached for up to an
           hour, so a session this page rightly treats as confirmed can still
           hold a token minted before it was, and every write bounces as
           permission-denied. See OAAccounts.freshClaims. */
        freshClaims(user).then(function () {
          ...
          return cvSlot.upload(user, function (pct) {
