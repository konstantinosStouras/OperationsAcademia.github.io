...
(function () {
  ...
  function paintAnswers() {
    ...
    if (open) {
      var host = ol.querySelector('li[data-pid="' + String(openFor).replace(/"/g, '\\"') + '"] .oa-prose');
      if (host) {
        host.hidden = true;
  ...
  function postHTML(p, thread, readOnly, isFirst, liveAnswers) {
    ...
    if (p.hidden) {
      ...
      out += '<div class="oa-prose">' + bodyHTML(p.body) + '</div>';
    }
  ...
  /* --------------------------------------- the formatting toolbar, borrowed

     The toolbar, the tips row, the keyboard shortcuts and the preview were
     written here (owner, 2026-09-08) and live in assets/oa-editor.js since
     2026-09-17, when the job posting's Comments box asked for the same
     editor: a copy beside that form would be two editors writing two
     dialects of one Markdown subset. The page calls the module and keeps
     what is its own -- the guard beside each box, which is a rule about an
     ANONYMOUS room and has no business on a job advertisement. */
  var ED = window.OAEditor;

  function toolbarHTML(taId, tipsId) { return ED.toolbarHTML(taId, tipsId); }
  function tipsHTML(id) { return ED.tipsHTML(id); }
  function previewHTML(id) { return ED.previewHTML(id); }
  function wireEditor(box, ta) { return ED.wire(box, ta); }
  ...
  function replyBox(thread, first) {
    ...
    wrap.innerHTML =
      ...
      '<div class="oa-editor">' +
        toolbarHTML('oa-forum-body', 'oa-editor-tips') +
        tipsHTML('oa-editor-tips') +
        '<textarea id="oa-forum-body" rows="6" maxlength="' + M.BOUNDS.body + '" placeholder="Answer the question. A few paragraphs at most." aria-label="Your answer" aria-describedby="oa-editor-tips"></textarea>' +
        previewHTML('oa-editor-preview') +
    ...
    wireEditor(wrap.querySelector('.oa-editor'), ta);
  ...
  function quoteFrom(li, p) {
    ...
    if (sel && !sel.isCollapsed && sel.anchorNode && li.querySelector('.oa-prose') &&
        li.querySelector('.oa-prose').contains(sel.anchorNode)) {
  ...
  function editPost(li, p) {
    var text = li.querySelector('.oa-prose');
    if (!text || li.querySelector('.oa-forum-editing')) return;
    var box = el('div', { class: 'oa-editor oa-forum-editing' });
    ...
    box.querySelector('[data-edit="cancel"]').addEventListener('click', function () {
      ...
      var body = box.parentNode && box.parentNode.querySelector('.oa-prose');
      box.remove();
