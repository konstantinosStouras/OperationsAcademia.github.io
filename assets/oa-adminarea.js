...
(function () {
  ...
  /* ------------------------------- postings that span two seasons: RETIRED

     There was a "Market year to check" panel here (owner, 2026-08-26): the
     build reported into data/jobs-yearcheck.json every posting whose stored
     season was behind what its own apply-by dates said, and the maintainer
     settled each one by hand.

     The overlap took its job away (owner, 2026-08-27). A posting is now
     listed under EVERY season it touches — the one it was advertised in, the
     one it is filed under, and the one its deadline falls in (`years`,
     marketYearsOf in _scraper/jobs-model.mjs) — so a stored season that is
     behind the dates no longer hides the posting from the season those dates
     name. There was nothing left to settle: the four postings the panel
     listed the day it was retired were all cases the overlap now serves under
     both seasons, and a list of postings doing exactly the right thing is
     worse than no list.

     Nothing replaced it, deliberately. If a panel is ever wanted here again,
     the question worth asking is the other one — which postings the overlap
     has widened — and it is answered by the `years` field the served file
     already carries.                                                        */
