...
| `userDirectory/{uid}` | the roster the Admin area lists: the name an account shows itself under, the address it signs in with, the affiliation on its profile, and when it registered (Auth's own creation time, filled by the daily sync) and was last seen. Written by the account about itself (and refreshed from Auth and the profiles by the daily sync), read by the **maintainer** (and by that account, about itself). `email` is pinned to `request.auth.token.email`, so a row cannot claim an address the account does not hold, and `first` is write-once. An account may delete **its own** row, as with the tally above. Disclosed in the Privacy Policy |
