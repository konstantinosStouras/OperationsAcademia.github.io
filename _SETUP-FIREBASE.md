...
| `registeredUsers/{uid}` | **maintainer-read** and contentless (one coarse timestamp — no name, no address), owner-write only; an account may delete **its own** mark, which is what a merge does with the duplicate it removes |
| `userDirectory/{uid}` | the roster the Admin area lists: the name an account shows itself under, the address it signs in with, the affiliation on its profile, the address it GAVE where its sign-in shares none (ORCID's does not, so `contactEmail` is the only way to reach those accounts), and when it registered (Auth's own creation time, filled by the daily sync) and was last seen. Written by the account about itself (and refreshed from Auth and the profiles by the daily sync), read by the **maintainer** (and by that account, about itself). `email` is pinned to `request.auth.token.email`, so a row cannot claim an address the account does not hold, and `first` is write-once. An account may delete **its own** row, as with the tally above. Disclosed in the Privacy Policy |
| `messages/{uid}` | one thread between the maintainer and that account, keyed on the uid — readable by those two and nobody else. The maintainer opens a thread and clears the flag; the owner may do exactly two things, mark it read and record a reply, and a reply may only RAISE the flag that says the maintainer owes an answer. `messages/{uid}/items/{id}` are the messages, with `from` pinned to whoever is writing, so neither side can put words in the other's mouth; only the maintainer may edit or retract one |
