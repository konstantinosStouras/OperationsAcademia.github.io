...
async function queuePass({ today, now, budget }) {
  ...
  for (const w of want) {
    ...
    const block = adBlock(entry, { adUrl: w.url });
...
async function main() {
  ...
  /* The queue pass runs after the published one so a busy morning spends its
     budget on what is already public first — a pending card is read within a
     day either way. */
