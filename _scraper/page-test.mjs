...
/* the legacy Awesome Table deep link the footer and the "Further info" column
   ...
   THE INSTITUTION IS DERIVED, never written down. Both this and the ?filterD=
   check below were pinned to "University of Mannheim", whose postings have
   since rolled out of the season the page shows — so they matched nothing,
   both assertions went red, and the `.oa-card` print check further down threw
   and took the whole suite with it. A browser check must not move with the
   corpus (the market-year fixture says so in as many words): the name comes
   from the served file, filtered by the pipeline's own window rule, so it is
   whatever the page is really showing on the day this runs. */
const inMarket = JSON.parse(await readFile(path.join(ROOT, 'data', 'jobs.json'), 'utf8'))
  .filter((r) => r && r.institution && inCurrentMarket(r));
const deepName = (inMarket[0] || {}).institution || '';
ok(deepName, 'the jobs page has something to deep-link to at all');
await page.goto(BASE + V2 + 'jobs.html?filterA=' + encodeURIComponent(deepName),
  { waitUntil: 'domcontentloaded' });
await page.waitForSelector('.oa-card, .oa-empty');
await page.waitForTimeout(300);
const mannheim = await page.$$eval('.oa-card-title', (ns) => ns.map((n) => n.textContent));
ok(mannheim.length > 0 && mannheim.every((t) => t.indexOf(deepName) !== -1),
  `the legacy ?filterA= deep link still selects an institution (${deepName})`);
...
// the Universities map deep-links institutions as ?filterD= — same name, and
// derived for the same reason as ?filterA= above
const viaDName = deepName;
await page.goto(BASE + V2 + 'jobs.html?filterD=' + encodeURIComponent(viaDName),
  { waitUntil: 'domcontentloaded' });
await page.waitForSelector('.oa-card, .oa-empty');
await page.waitForTimeout(300);
const viaD = await page.$$eval('.oa-card-title', (ns) => ns.map((n) => n.textContent));
ok(viaD.length > 0 && viaD.every((t) => t.indexOf(viaDName) !== -1),
  `the Universities map's ?filterD= deep link selects an institution (${viaDName})`);
