   ...
   390px block rule 13 in _MOBILE-STANDARDS.md is measured by. */
{
  ...
  {
    const { ctx, page: q, errors } = await signedInPage('forum.html?room=candidates',
        ...
        once per device per day --------------------------------------------- */
  {
    ...
    eq(home.hrefs, ['forum.html?room=candidates', 'forum.html?room=open'], 'forum (sections): each door is a link to its room\'s questions');
        ...
        the memory and says why ---------------------------------------------- */
  {
    ...
    {
      const { ctx: c2, page: q2, errors: e2 } = await signedInPage('forum.html?room=candidates',
    ...
    {
      const { ctx: c3, page: q3, errors: e3 } = await signedInPage('forum.html?room=candidates',
  ...
  /* -- the maintainer, with no profile: both rooms, the guide seeded, a post -- */
  {
    const ADMIN_USER = { uid: 'admin-uid-0000000000', email: 'kstouras@gmail.com',
      emailVerified: true, displayName: 'Kostas Stouras', providerData: [] };
    const { ctx, page: q, errors } = await signedInPage('forum.html?room=candidates', { user: ADMIN_USER, selector: '#oa-forum' });
  ...
  {
    const { ctx, page: m, errors } = await signedInPage('forum.html?room=candidates',
