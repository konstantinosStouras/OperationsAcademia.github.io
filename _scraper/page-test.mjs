   ...
   this stays green as the site's own data moves. */
{
  ...
  /* -- the desk itself ------------------------------------------------------ */
  {
    ...
    /* the tiles and the badge, corrected from the documents on screen — the
       ...
       ticked off above, and nothing recounts them mid-session), and the
       approved name fix no longer counts as waiting */
    await q.waitForFunction((want) => {
    ...
    }, ['5', seededHeld, '2', newsPending, 0].join(','), { timeout: 10000 });
